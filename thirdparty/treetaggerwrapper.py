...
STARTOFTEXT = b"<This-is-the-start-of-the-text />"
ENDOFTEXT = b"<This-is-the-end-of-the-text />"
...
g_langsupport = {
    ...
    "ru": { "binfile-win": "tree-tagger.exe",
			...
			"dummysentence": "Съешь этих мягких французских булок, да выпей же чаю.",
			"replurlexp": r' replaced-url <repurl text="\1" />',
			"replemailexp": r' replaced-email <repemail text="\1" />',
			"replipexp" : r' replaced-ip <repip text="\1" />',
			"repldnsexp" : r' replaced-dns <repdns text="\1" />'
...
class TreeTagger (object) :
    ...
    def TagText(self,text,numlines=False,tagonly=False,
                ...
                encoding='utf-8',errors=DEFAULT_ENCERRORS) :
        ...
        # Manage encoding.
        use_unicode = True#isinstance(text,str) or (encoding in(str,"unicode"))
        ...
        t.start()
        #time.sleep(0.1) # Leave thread and tagger time to start communicating.
        ...
        while True :
            ...
            if bline == STARTOFTEXT :
                intext = True
                continue
            if bline == ENDOFTEXT :  # The flag we sent to identify texts.
                intext = False
                break
            if intext and line :
